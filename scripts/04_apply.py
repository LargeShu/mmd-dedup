...
обычным способом.
    ...
    #    Имя файла — целиком, без шаблона *: за прогон обрабатывается один
    #    файл решений, и выбирать его должен человек, а не оболочка.
    python3 04_apply.py --decisions ../reports/decisions_20260810-231500.json
...
В карантин рядом с файлами кладутся три вещи:

  * journal.csv    — что перенесено: откуда, куда, размер, хеш;
  * run_info.txt   — чем это сделано: версия, время, файл решений и его
                     sha256, идентификатор отчёта, команда для отката;
  * decisions.json — копия самих решений.

Журнал говорит, ЧТО перенесено, но не говорит, по какому решению. Карантин
живёт неделями, а исходный файл решений к тому времени бывает переименован
или удалён из Загрузок — поэтому карантин обязан объяснять себя сам.

Журналов столько, сколько задействовано дисков, поэтому --undo принимает
и папку карантина целиком.
"""
...
JOURNAL = "journal.csv"
# Рядом с журналом кладём происхождение прогона. Карантин живёт неделями,
# а вопрос «откуда это и по какому решению» возникает позже, когда файл
# решений уже переименован, перезаписан или удалён из Загрузок.
# Имена латиницей: карантин лежит на сетевом томе, а SMB, ExFAT и чужие
# ОС по-разному обходятся с кириллицей в именах файлов. Содержимое
# по-русски, имя — нет.
RUNINFO = "run_info.txt"
DECISIONS_COPY = "decisions.json"
...
def записать_происхождение(base, decisions_path, d, всего, verify_hash):
    """
    Кладёт в карантин run_info.txt и копию решений.

    Без этого карантин — папка с файлами непонятного происхождения: журнал
    говорит, ЧТО перенесено, но не говорит, по какому решению и каким
    отчётом оно принято. Через неделю восстановить это неоткуда.
    """
    try:
        with open(os.path.join(base, RUNINFO), "w", encoding="utf-8") as fh:
            fh.write(f"{L.version_line()}\n")
            fh.write(f"прогон:        {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
            fh.write(f"файл решений:  {os.path.abspath(decisions_path)}\n")
            fh.write(f"  размер:      {os.path.getsize(decisions_path):,} байт\n")
            fh.write(f"  sha256:      {sha256(decisions_path)}\n")
            fh.write(f"отчёт:         {d.get('отчёт', '?')}\n")
            fh.write(f"решения сохранены: {d.get('сохранено', '?')}\n")
            fh.write(f"решений в файле:   {всего:,}\n")
            fh.write(f"сверка хешей:  {'да' if verify_hash else 'нет'}\n")
            fh.write("\nВернуть всё обратно:\n")
            fh.write(f"  python3 04_apply.py --undo \"{base}\"\n")
        # Копия самих решений: исходный файл переименуют или сотрут,
        # а карантин должен оставаться самодостаточным.
        shutil.copy2(decisions_path, os.path.join(base, DECISIONS_COPY))
    except OSError as e:
        print(f"  ! не удалось записать происхождение прогона: {e}")


def apply_move(годные, bases, drives, verify_hash, quiet=False,
               decisions_path=None, d=None, всего=0):
    """
    ...
    try:
        for i, r in enumerate(годные, 1):
            ...
            if имя_диска not in журналы:
                ...
                if decisions_path:
                    записать_происхождение(base, decisions_path, d or {},
                                           всего, verify_hash)
...
def печать_итога(n, err, байт, сек, журналы, bases, drives, старт,
    ...
    print("=" * 66)
    if decisions_path:
        print(f"  файл решений : {os.path.abspath(decisions_path)}")
...
def main():
    ...
    a, лишние = ap.parse_known_args()
    # Оболочка раскрывает шаблон: --decisions ../reports/decisions_*.json
    # при нескольких совпадениях отдаёт первый файл флагу, а остальные
    # оставляет висеть. Argparse ругался «unrecognized arguments», и человек
    # не понимал, что половина решений просто не попала в работу.
    if лишние:
        json_лишние = [x for x in лишние if x.lower().endswith(".json")]
        if json_лишние:
            raise SystemExit(
                "Под шаблон попало несколько файлов решений:\n"
                + f"  взят в работу: {a.decisions}\n"
                + "".join(f"  ПРОПУЩЕН:      {x}\n" for x in json_лишние)
                + "\nЗа один прогон обрабатывается ОДИН файл решений: иначе\n"
                  "непонятно, по какому из них собран карантин. Укажите файл\n"
                  "явно или обработайте их по очереди.")
        ap.error("неизвестные аргументы: " + " ".join(лишние))
    ...
    print(f"файл решений: {os.path.abspath(a.decisions)}")
    ...
    n, err, байт, сек, журналы = apply_move(
        годные, bases, drives, a.verify_hash, a.quiet,
        decisions_path=a.decisions, d=d, всего=len(rows))
    печать_итога(n, err, байт, сек, журналы, bases, drives, старт,
                 decisions_path=a.decisions)
    # Напоминание печатается здесь, а не только в README: про этот шаг
    # забывают ровно в тот момент, когда перенос закончился и кажется,
    # что дело сделано. База при этом продолжает считать перенесённые файлы
    # существующими, и следующий отчёт покажет те же дубли.
    print("\n" + "-" * 66)
    print("СЛЕДУЮЩИЙ ШАГ — иначе отчёт покажет уже убранные дубли:")
    print("  1. убрать их из базы:   python3 01_inventory.py --prune")
    print("  2. пересобрать отчёт:   python3 03_report.py")

    print("\nВ карантине рядом с файлами: journal.csv (что перенесено), "
          + RUNINFO + " и " + DECISIONS_COPY + " (по каким решениям).")
