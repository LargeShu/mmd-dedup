...
def точка_монтирования(path: str) -> str:
    """
    Каталог, на котором меняется устройство, — граница тома.

    os.path.ismount не годится: он отвечает только про сам путь, а корень
    диска в config.json может лежать ВНУТРИ тома. У пользователя
    «/Volumes/video/300 Photos» — это папка на томе «/Volumes/video»,
    и подниматься надо до смены st_dev.
    """
    ...
    try:
        ...
        return p
    while True:
        родитель = os.path.dirname(p)
        if родитель == p:
            return p
        try:
            if os.stat(родитель).st_dev != dev:
                return p
        except OSError:
            return p
        p = родитель
...
def проверить_диски(drives: dict, строго: bool = False):
    """
    Диски из config.json действительно подключены?

    Возвращает (стоп, внимание) — два списка строк. Непустой «стоп» означает
    «работать нельзя».

    Почему одной проверки isdir мало. Когда сетевая шара не примонтирована,
    macOS оставляет на её месте ПУСТОЙ ЛОКАЛЬНЫЙ КАТАЛОГ с тем же именем.
    isdir на нём проходит, и дальше начинается тихая беда: этап 1 видит ноль
    файлов и при --prune вычищает инвентарь; этап 4 «переносит» дубли
    на системный диск вместо хранилища. Оба случая выглядят как успех.

    Пустой каталог — подозрение, а не приговор: диск может быть пуст законно.
    Поэтому он попадает в «стоп» только при строго=True — там, где цена
    ошибки высока: перенос файлов и --prune. Отказ на ровном месте так же
    вреден, как и молчание: от него отучаются флагом, и тогда проверка
    перестаёт защищать вообще.

    А вот «путь ведёт в /Volumes, но том системный» — приговор всегда:
    примонтирована не шара, а её пустой заместитель.
    """
    системный = os.stat(os.sep).st_dev
    стоп, внимание = [], []
    for имя, корень in sorted(drives.items()):
        if not os.path.exists(корень):
            стоп.append(f"{имя}: каталога нет — {корень}")
            continue
        if not os.path.isdir(корень):
            стоп.append(f"{имя}: это не каталог — {корень}")
            continue
        try:
            пусто = not any(os.scandir(корень))
        except OSError as e:
            стоп.append(f"{имя}: не читается ({e.__class__.__name__}) — {корень}")
            continue
        свой_том = os.stat(корень).st_dev != системный
        внешний = os.path.realpath(корень).startswith(
            ("/Volumes/", "/mnt/", "/media/"))
        if внешний and not свой_том:
            стоп.append(
                f"{имя}: путь ведёт на внешний том, но лежит на системном "
                f"диске — {корень}\n"
                f"      Шара не примонтирована; работа шла бы мимо архива.")
            continue
        if пусто:
            текст = (f"{имя}: каталог ПУСТ — {корень}\n"
                     f"      Если это сетевая шара, она, похоже, "
                     f"не примонтирована.")
            (стоп if строго else внимание).append(текст)
    return стоп, внимание


def сообщить_о_дисках(drives: dict, строго: bool = False) -> bool:
    """
    Печатает состояние дисков. Возвращает False, если работать нельзя.

    Общая для всех этапов, чтобы формулировки и порог срабатывания
    не разъезжались по скриптам.
    """
    print("диски:")
    for строка in описание_дисков(drives):
        print(f"  {строка}")
    стоп, внимание = проверить_диски(drives, строго)
    for w in внимание:
        print(f"  ! {w}")
    if стоп:
        print("\n!! ДИСКИ НЕ ГОТОВЫ:")
...
def описание_дисков(drives: dict) -> list[str]:
    """Строки «имя → путь (том, свободно)» для вывода перед работой."""
    out = []
    for имя, корень in sorted(drives.items()):
        try:
            точка = точка_монтирования(корень)
            св = shutil.disk_usage(корень).free
            out.append(f"{имя}: {корень}  ·  том {точка}  ·  свободно {human(св)}")
        except OSError as e:
            out.append(f"{имя}: {корень}  ·  недоступен ({e.__class__.__name__})")
    return out
...
def merge_roots(cfg_drives: dict, root_args) -> dict:
    """
    Диски: из config.json либо ЦЕЛИКОМ из флагов --root ИМЯ=ПУТЬ.

    Раньше флаги «дополняли» конфигурацию, и это оказалось ловушкой.
    Запуск на паре тестовых каталогов молча тащил за собой настоящие диски
    из config.json: они попадали в проверки, в план, в выбор оригинала.
    Человек, набравший --root, имеет в виду «работай вот с этим», а не
    «прибавь к моему архиву».

    Правило простое: есть хоть один --root — работаем ровно по ним.
    """
    if not root_args:
        return dict(cfg_drives)
    out = {}
